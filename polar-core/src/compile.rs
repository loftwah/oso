    ...
    A: JsString,
{
    fn to_js(&self) -> String {
        format!(
            "[{}]",
            self.iter().map(|x| x.to_js()).collect::<Vec<_>>().join(",")
        )
    }
}
...
impl JsString for Rule {
    fn to_js(&self) -> String {
        ...
        let formal_params = self
            .params
            .iter()
            .map(|p| match p.parameter.value().as_symbol() {
                Ok(s) => s.clone(),
                _ => {
                    let sym = gensym();
        ...
        format!(
            ...
            all_vars
                ...
                .map(|s| lits
                    .remove(s)
                    .map_or_else(|| String::from("(new Var())"), |t| t.to_js()))
                .collect::<Vec<_>>()
                .join(",")
        )
    }
...
impl JsString for bool {
    fn to_js(&self) -> String {
        format!("{}", self)
    }
}
impl JsString for String {
    fn to_js(&self) -> String {
        format!("{:?}", self)
    }
}
impl JsString for i64 {
    fn to_js(&self) -> String {
        format!("{}", self)
    }
}
impl JsString for f64 {
    fn to_js(&self) -> String {
        format!("{}", self)
    }
}
impl JsString for Symbol {
    fn to_js(&self) -> String {
        self.0.clone()
    }
}
